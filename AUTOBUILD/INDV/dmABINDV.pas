...
type
  ...
  TABINV = class(TDataModule)
    ...
    qryCountNominees: TFDQuery;
    qryHeatMaxHeatNum: TFDQuery;
    tbl_ABHeat: TFDTable;
    tbl_ABLane: TFDTable;
...
procedure TABINV.ActivateData;
begin
  IsActive := false;
  //  procDeleteHeats.Connection := SCM2.scmConnection;
  if Assigned(SCM2) and SCM2.scmConnection.Connected then
  begin
    qryCountNominees.Connection := SCM2.scmConnection;
    IsActive := true;
  end;
end;

function TABINV.AssertConnection: boolean;
begin
  result := false;
...
function TABINV.AssignHeats(EventID, numOfSwimmingLanes, GroupBy,
  Unplaced: integer; SeperateGender: boolean): integer;
var
  Success: boolean;
...
function TABINV.AssignLane(DataSet: TFDQuery;
...
begin
  ...
  begin
    SearchOptions := [];
    result := tbl_ABLane.Locate('HeatID; Lane',
      VarArrayOf([HeatID, laneNumber]), SearchOptions);
    if (result) then
    begin
      tbl_ABLane.Edit;
      tbl_ABLane.FieldByName('MemberID').AsInteger :=
      ...
      *)
      tbl_ABLane.FieldByName('TimeToBeat').AsDateTime :=
        DataSet.FieldByName('TTB').AsDateTime;
      tbl_ABLane.FieldByName('PersonalBest').AsDateTime :=
      ...
      *)
      ...
      begin
        case prefImportSeedTime of
          1: // RaceTime
            tbl_ABLane.FieldByName('RaceTime').AsDateTime :=
...
function TABINV.AssignLanes(DataSet: TFDQuery;
  HeatID, numOfNomineesInHeat: integer): integer;
var
  pass: boolean;
...
// ***********************************************************************
function TABINV.AssignNominees(DataSet: TFDQuery;
...
function TABINV.CircleSeed(DataSet: TFDQuery; seedDepth: integer)
  : integer;
var
...
function TABINV.CreateNewEmptyHeat(EventID: integer): integer;
var
  SearchOptions: TLocateOptions;
...
begin
  ...
  NextHeatNum := GetHeatMaxHeatNum(EventID) + 1;
  ...
  begin
    ...
    begin
      ...
      tbl_ABLane.FieldByName('HeatID').AsInteger :=
        tbl_ABHeat.FieldByName('HeatID').AsInteger;
      // Assign lane number
      tbl_ABLane.FieldByName('Lane').AsInteger := (iterLanes + 1);
      // Assert bit initialisation  BSA FIX 20240309
      tbl_ABLane.FieldByName('IsDisqualified').AsBoolean:= false;
      tbl_ABLane.FieldByName('IsScratched').AsBoolean:= false;
      tbl_ABLane.Post;
    end;
    tbl_ABLane.Close;
    result := HeatID;
...
function TABINV.CreateNewEvent(SourceEvent: TFDQuery;
  TypeOfFinals: scmEventFinalsType): integer;
var
  s: string;
begin
...
procedure TABINV.DataModuleCreate(Sender: TObject);
var
  iniFileName: string;
begin
  ...
  //qryOrderedHeatList.Connection := SCM2.scmConnection;
...
procedure TABINV.DeActivateData;
begin
    //  procDeleteHeats.Connection := SCM2.scmConnection;

end;

function TABINV.GetHeatMaxHeatNum(EventID: integer): integer;
var
  ds: TFDQuery;
begin
  result := 0;

  ds := qryHeatMaxHeatNum;
  ds.Close;
  ds.ParamByName('EVENTID').AsInteger := EventID;
  ds.Prepare;
  ds.Open;
...
procedure TABINV.ReadPreferences(iniFileName: string);
var
  iFile: TIniFile;
  i: integer;
begin
...
{
function TAutoBuildV2.OrderedHeatList(EventID: integer): integer;
var
  ds: TFDQuery;
  iter: integer;
begin
...
function TABINV.SeedNominees(DataSet: TFDQuery; mySeedMethod: seedMethod;
